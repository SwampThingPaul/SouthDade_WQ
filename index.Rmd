...
map.dat=data.frame(SITE=c("S331", "S173", "S357", "S332B", "S332C", "S174", "S332D", 
...
### [`r paste(fontawesome::fa("github"),"Source Code")`](https://github.com/owper-tech/SouthDade_WQ){target="_blank"}

* GitHub repo [https://github.com/owper-tech/SouthDade_WQ](https://github.com/owper-tech/SouthDade_WQ){target="_blank"}

***

**_Please note this analysis is for informational purposes only and subject to change._** 

Data and information presented herein is purely for information purposes and subject to change. This information is intended to provide a retrospective look at water quality (i.e. total phosphorus) and water quantity within the southern Miami-Dade County region which spans Everglades National Park eastern boundary. 
...
Provided in this section are period of record (POR) discharge data and associated metrics of available data in the South Florida Water Management Districts [DBHYDRO](http://my.sfwmd.gov/dbhydroplsql/show_dbkey_info.main_menu){target="_blank"}. All structure DBKEYs used to construct these discharge time-series can be downloaded below.
...
```{r,include=F}
flow.Sites=c("S331","S173","S357","S332B","S332C","S174","S332D","S332DX1","S328","S332","S176","S200","S332","S175","S199","S177","S178","S18C","S197")
```
...
for(i in 1:length(flow.Sites)){
  ...
  abline(h=ymaj,v=xmaj,lty=3,col="grey")
  with(dat.WY,shaded.range(WY,rep(0,nrow(dat.WY)),Q.m3d,bg="dodgerblue1",col="black",lty=1,col.adj = 0.5,lwd=0.5))
  ...
  cat("<br>")
  cat("Daily Discharge Data Summary for the Period of Record (Units:m^3^ d^-1^)")
  cat("<br>")
  
  dat_summary=ddply(dat.da,"SITE",summarise,
          Min=min(Q.m3d,na.rm=T),
          Q1=quantile(Q.m3d,na.rm=T,probs=0.25),
          Median=median(Q.m3d,na.rm=T),
          Q3=quantile(Q.m3d,na.rm=T,probs=0.75),
          Max=max(Q.m3d,na.rm=T))
  
  print(kable(dat_summary,align=c("c"),row.names = F,escape=F)%>%
    kable_styling(bootstrap_options = "striped", full_width = F))
  
  cat(paste0("Daily Discharge Data Summary for the last 10-WYs (May ", WY(dates[2])-11," - April ", WY(dates[2]),"; Units:m^3^ d^-1^)"))
  cat("<br>")
  
  dat_summary=ddply(subset(dat.da,WY%in%seq(WY(dates[2])-11,WY(dates[2]),1)),"SITE",summarise,
          Min=min(Q.m3d,na.rm=T),
          Q1=quantile(Q.m3d,na.rm=T,probs=0.25),
          Median=median(Q.m3d,na.rm=T),
          Q3=quantile(Q.m3d,na.rm=T,probs=0.75),
  ...
  cat("Water Year Discharge Data Summary for the Period of Record (Units:m^3^ WY^-1^)")
  cat("<br>")
  dat_summary=ddply(dat.WY,"SITE",summarise,
          Min=min(Q.m3d,na.rm=T),
          Q1=quantile(Q.m3d,na.rm=T,probs=0.25),
          Median=median(Q.m3d,na.rm=T),
          Q3=quantile(Q.m3d,na.rm=T,probs=0.75),
  ...
  cat(paste0("Water Year Discharge Data Summary for the last 10-WYs (May ", WY(dates[2])-11," - April ", WY(dates[2]),"; Units:m^3^ WY^-1^)"))
  cat("<br>")
  
  dat_summary=ddply(subset(dat.WY,WY%in%seq(WY(dates[2])-11,WY(dates[2]),1)),"SITE",summarise,
          Min=min(Q.m3d,na.rm=T),
          Q1=quantile(Q.m3d,na.rm=T,probs=0.25),
          Median=median(Q.m3d,na.rm=T),
          Q3=quantile(Q.m3d,na.rm=T,probs=0.75),
